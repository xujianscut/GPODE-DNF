...
def initialize_inducing(model, data_ys, ts_max, x,data_noise=1e0):
    """
    ...
    with torch.no_grad():
        ...
        inducing_loc = torch.tensor(kmeans2(data_ys, k=model.flow.odefunc.diffeq.Um(x).shape[0], minit='points')[0])
        ...
        model.flow.odefunc.diffeq.Um(x).data = inducing_val.data  # (M,D)
        return model


def initialize_latents_with_data(model, data_ys, data_ts,x, num_samples=50):
    """
    ...
    Other states are initialization at observed values.
    ...
    with torch.no_grad():
        ...
        for _ in range(num_samples):
            init_x0.append(model.build_flow(init_xs[:, 0], init_ts,x).clone().detach().data[:, -1])
